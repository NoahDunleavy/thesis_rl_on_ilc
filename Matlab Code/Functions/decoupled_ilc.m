function [ILC_Trial, F_decoupled, controller_error, F_lqr] = decoupled_ilc(P, d, C, D, x0, y_star, gamma, Q, R, num_controllers, exploration_mag, input_basis_functions, output_basis_functions, num_converged)
%Perform the RL input decoupled learning on an ILC system since done so much in
%thesis
%Inputs:
...
%Outputs:
    %ILC_Trial: structure with indexed by trial number, contains
        ...
        %output error (y* - y)
        %alpha error
    %F_decoupled: matrix - controller learning history
    %controller_error: scalar - normalized error from LQR
    %F_lqr: matrix - lqr controller to compare to
...
%Iteration Counts
Pj_dim = num_ilc_states + 1;
num_collections_per_controller = Pj_dim^2;    
total_trial_count = num_ilc_inputs * num_controllers * num_collections_per_controller + num_converged;

%Preallocate structure
ILC_Trial(total_trial_count).input = [];  %input
...
F_decoupled = zeros(num_ilc_inputs, num_ilc_states, num_controllers + 1);   %start with no controller

%Prepopulate the first trial
trial_num = 1;
ILC_Trial_decoupled(trial_num).betas = zeros(num_ilc_inputs, 1);   %start with no basis guessed
ILC_Trial_decoupled(trial_num).input = input_basis_functions * ILC_Trial_decoupled(trial_num).betas; 

ILC_Trial_decoupled(trial_num).output = [C*x0; d];    %open loop response is IC and then d term
ILC_Trial_decoupled(trial_num).alphas = output_basis_functions_pinv * d;
ILC_Trial_decoupled(trial_num).output_error = y_star - d;    %relevant error
ILC_Trial_decoupled(trial_num).alpha_error = alpha_star - ILC_Trial_decoupled(trial_num).alphas;    
...
for iteration = 1:num_controllers
    for input_num = 1:num_ilc_inputs
        ...
        for trial = 1:num_collections_per_controller   %number of trials to collect before updatin controller
            ...
            %Beta Coeffecients
            exploration_term = rand_range(1, 1, -exploration_mag, exploration_mag);    %jiggle to learn

            ILC_Trial_decoupled(trial_num).del_beta = F_decoupled(:, :, end) * ILC_Trial_decoupled(trial_num - 1).alpha_error;
            ILC_Trial_decoupled(trial_num).del_beta(input_num) = ILC_Trial_decoupled(trial_num).del_beta(input_num) + exploration_term;

            ILC_Trial_decoupled(trial_num).betas = ILC_Trial_decoupled(trial_num - 1).betas + ILC_Trial_decoupled(trial_num).del_beta;
            %Beta to Inputs
            ILC_Trial_decoupled(trial_num).input = input_basis_functions * ILC_Trial_decoupled(trial_num).betas;
            %Simulate Reality
            relevant_output = P * ILC_Trial_decoupled(trial_num).input + d;   %y(1) -> y(p)
            ILC_Trial_decoupled(trial_num).alphas = output_basis_functions_pinv * relevant_output;
            ...
            %RL Translation
            state = ILC_Trial_decoupled(trial_num - 1).alpha_error; %analogous state
            full_input = ILC_Trial_decoupled(trial_num).del_beta;        %analogous input
            input = full_input(input_num);
            next_state = ILC_Trial_decoupled(trial_num).alpha_error;    %x(k+1) = e_alpha(j)
        ...
        F_decoupled(:, :, end + 1) = F_decoupled(:, :, end);
        F_decoupled(input_num, :, end) = new_F_i;
    end
end
...
for ndx = 1:num_converged
    %ILC / Basis Controller Process
    %Beta Coeffecients
    ILC_Trial_decoupled(trial_num).del_beta = F_decoupled(:, :, iteration) * ILC_Trial_decoupled(trial_num - 1).alpha_error;
    ILC_Trial_decoupled(trial_num).betas = ILC_Trial_decoupled(trial_num - 1).betas + ILC_Trial_decoupled(trial_num).del_beta;
    %Beta to Inputs
    ILC_Trial_decoupled(trial_num).input = input_basis_functions * ILC_Trial_decoupled(trial_num).betas;
    %Simulate Reality
...
controller_error = norm(F_decoupled(:, :, end) - F_lqr)/numel(F_lqr);
