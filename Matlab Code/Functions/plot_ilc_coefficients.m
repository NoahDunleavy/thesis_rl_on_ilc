function [alpha_error_fig, beta_error_fig, alpha_prog_fig, beta_prog_fig] = plot_ilc_coefficients(figure_name, graph_title, ILC_Trial, to_plot, beta_star, betas_to_plot, alphas_to_plot, save_path)
%Note: beta star error is sometimes illogical
%Plot out the progression of coefficients in an ILC problem described in a
%basis space
%Inputs:
...
%Determine which coefficients to plot
...
%Progression of errors on coefficients
...
semilogy(1:num_trials, alpha_error_norm, 'r*', 'LineStyle',':');
...
if beta_star_passed %if we were given a beta_star
    ...
    semilogy(1:num_trials, beta_error_norm, 'r*', 'LineStyle',':');
    set(gca, 'YScale', 'log'); %force log scale (sometimes linear if small range)
...
alpha_prog_fig = figure('Name', sprintf('%s - Alpha Coefficient Progression', figure_name));
...
beta_prog_fig = figure('Name', sprintf('%s - Beta Coefficient Progression', figure_name));
legend_list = "[REMOVE ME]";
hold on;
